...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        def lol(id,name,user):
            connector = sqlite3.connect('C:/Users/obikf/PycharmProjects/rem_bot/rem_bot_proj/db.sqlite3')
            ...
            connector.commit()
            #except:
                #cursor.execute('''update rem_bot_profile set username = ? where external_id = ? ''',(user, id))
                #cursor.execute('''update rem_bot_profile set name = ? where external_id = ? ''', (name, id))
                #connector.commit()
        ...
        async def send_hello(message: types.Message):
            ...
            await message.answer('Привет! Я Remover_Bot и я создан для того, что бы удалять задний фон на фото. Отправь мне фото '
                                 'и я удалю задний фон или размою его'),lol(message.from_user.id,message.from_user.first_name,message.from_user.username)
        ...
        async def take_photo(message: types.Message):
            ...
            await message.photo[-1].download(f'media/{message.from_user.id}.jpg'), remove_bg(message.from_user.id), portrait(message.from_user.id)
            await paste(message.from_user.id),lol(message.from_user.id,message.from_user.first_name,message.from_user.username)
        ...
        @dp.message_handler(regexp='Получить фото фото без фона')
        async def send_photo(message: types.Message):
            markup = types.ReplyKeyboardRemove()
